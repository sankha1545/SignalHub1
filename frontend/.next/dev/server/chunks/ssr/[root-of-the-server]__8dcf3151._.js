...
function SignupPage() {
    ...
    function validatePassword(pw) {
        ...
        if (!/[^\w\s]/.test(pw)) errs.push("At least one symbol (e.g. !@#$%)");
        return errs;
    }
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
    ...
    async function handleFinalize(e) {
        e?.preventDefault();
        setError(null);
        // Make sure password rules are satisfied and confirm matches (password is now mandatory)
        ...
        setLoading(true);
        try {
            // Send all collected profile data to finalise signup so backend can store profile
            const payload = {
                tempToken,
                organizationName: orgName.trim(),
                password,
                name,
                // include resolved contact details (so backend can store profile fields)
                email: email || undefined,
                phone: phone && (phone.startsWith("+") ? phone : `${country?.callingCode ?? ""}${phone.replace(/\D/g, "")}`) || undefined,
                country: country ? {
                    name: country.name,
                    cca2: country.cca2,
            ...
            } else setError(res?.error || "Failed to finalize signup.");
        } catch (err) {
            setError(String(err));
        } finally{
            setLoading(false);
