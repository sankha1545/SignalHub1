 ...
 */ /* ------------------------- Helpers ------------------------- */ function routeLog(...args) {
 ...
 */ async function createOrGenerateOtp(phone) {
    try {
        // dynamic import so route remains resilient if helper is absent or path differs
        const otpModule = await __turbopack_context__.A("[project]/src/lib/otp.ts [app-route] (ecmascript, async loader)").catch(()=>null);
        if (otpModule && typeof otpModule.createPhoneOtp === "function") {
            try {
                ...
                if (!code) {
                    routeLog("createPhoneOtp returned invalid shape; falling back to generated code", {
                        returned: res
                    });
                } else {
                    return {
...
async function POST(req) {
    try {
        ...
        if (tempToken) {
            try {
                const jwtMod = await __turbopack_context__.A("[project]/src/lib/jwt.ts [app-route] (ecmascript, async loader)").catch(()=>null);
                if (jwtMod && typeof jwtMod.verifyTemp === "function") {
                    try {
                        ...
                        if (decoded?.phone && decoded.phone !== phone) {
                            ...
                            routeLog("Warning: tempToken phone mismatch", {
        ...
        if (TW_SID && TW_TOKEN && TW_FROM) {
            ...
            try {
                const Twilio = (await __turbopack_context__.A("[project]/node_modules/twilio/lib/index.js [app-route] (ecmascript, async loader)")).default;
                ...
                routeLog("Twilio message created", {
                    sid,
                    status,
                    to: phone
                });
            ...
            devOtpLogged = true;
            routeLog("Twilio not configured — dev OTP logged", {
                phone,
                id: id ?? null,
                code,
                messageText
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0d726a34._.js.map